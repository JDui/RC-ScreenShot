...
- 支持长截图：按内容逐帧对齐的滚动拼接（上/下/右三个方向），无重影、无重复内容。
...
- 截图中按空格循环普通、窗口、单元模式。
- 完成框选后，拖动边框或四角/边中把手可直接调整截图范围。
- 长截图：点击工具栏的长截图按钮，工具栏翻转为长截图功能栏，选择滚动方向（上/右/下）后以选区中央为锚点自动滚动拼接，`Enter` 结束预览并按默认动作输出，`Esc` 逐级返回。
...
选择文字工具后点击选区输入内容，输入前显示半透明“输入文字”提示；支持多行，`Enter` 换行、`Ctrl+Enter` 确认、`Esc` 取消，输入中可点击文字任意位置移动光标，双击已有文字可重新编辑；横排/竖排可在参数栏或右键菜单切换。
...
- 当前版本不包含 OCR、云上传、截图历史和 ARM64。
- 长截图依赖窗口响应滚轮事件：不响应 `WM_MOUSEWHEEL` 的应用（部分游戏和自绘画布）无法自动滚动拼接。
